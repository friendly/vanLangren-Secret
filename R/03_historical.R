# Reproducing van Langren's graph 
# From Cédric Scherer, #30DayChartChallenge 2021 Day 3: Historical
# https://github.com/z3tt/30DayChartChallenge2021/blob/main/03_historical/03_historical.R
# Lightly edited for use here

library(tidyverse)
library(ggtext)
library(ragg)

# fonts: download from Google Fonts (if not installed) and register with
# systemfonts for this session, so ragg and ggtext can find them
systemfonts::require_font("Averia Serif Libre")
systemfonts::require_font("Kaushan Script")
...
p <- ggplot() +
  ...
  scale_y_continuous(limits = c(.4, 2.7), expand = c(0, 0)) +
  # credit line, flush right below the axis
  geom_richtext(aes(x = 30.05, y = .52,
                    label = "by Cédric Scherer | <i>#30DayChartChallenge 2021</i> | <i>Day 3: Historical</i>"),
                hjust = 1, size = 5.3, family = "Averia Serif Libre", color = "grey15",
                fill = NA, label.colour = NA) +
  theme_void()

ggsave(here::here("images", "03_historical.png"), plot = p,
       width = 17600, height = 4600, units = "px", dpi = 1200,
