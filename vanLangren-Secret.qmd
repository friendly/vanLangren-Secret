...
What interested us most was *why* van Langren showed the earlier estimates in what would be the **first graphical display of quantitative data**, a 1D dot plot, rather than the table anyone else at the time would have used. I always ask:
...
![van Langren's 1644 graph showing 12 estimates of the longitude distance from Toledo to Rome by eminent astronomers, overlaid on a modern map. Map data © Google. _Source_: [@Friendly:2010:langren].](images/Langren1644-google-overlay.jpg){#fig-1644-overlay}
...
He never said what the answer was. On page 8 of *La Verdadera* he printed it anyway---but in cipher (@fig-cipher-page), promising to reveal the plaintext after suitable compensation. Except for the puzzle, the text of *La Verdadera* reads like a modern grant application.
We reproduced the ciphertext in our supplementary materials [@Friendly:2010:langren-supp] and moved on---though not before a few retired CIA and NASA codebreakers I tracked down in 2009--10 gave it a real try. Van Langren beat them all.

![The printed cipher from *La Verdadera Longitud por Mar y Tierra* (1644) page 8, exactly as van Langren's readers first saw it. _Source_: Courtesy of The Newberry Library, Chicago. Ayer folio QB225 .L36 1644.](images/verdadera-cipher-Newberry-bw.png){#fig-cipher-page width="80%"}
...
I assumed, as I think most people who have written about van Langren did, that his secret had to do with the achievement he's remembered for: his 1645 lunar map [@Langren:1645], and the method he built around it---using the sunrise and sunset lines crossing named lunar peaks as a celestial clock. It was a reasonable guess, and it made a good (his)story. It was also wrong.
...
Eleven years after our paper---long enough that I had filed the cryptogram away as a loose end nobody would ever tie off---I learned that it had been solved. Not by a historian of science, but by a Belgian warehouse worker and amateur cryptographer. Other amateurs had kept it alive: the cryptology blogger Klaus Schmeh flagged it in 2015, and the mathematician Dirk Huylebrouck spent years shopping it around online forums. Then, in December 2020, Jarl Van Eycke, with David Oranchak and Sam Blake, cracked the Zodiac Killer's Z340 cipher [@Oranchak:2024],[^zodiac] unsolved for 51 years. Huylebrouck tracked down Van Eycke and handed him the _van Langren Code_. He solved it in about a week!
...
Van Eycke never published his own account of the break; what follows retraces his steps as Klaus Schmeh reported them [@Schmeh:2021] from Van Eycke's email, applied to the ciphertext from our 2010 supplementary materials. It's a small master class in breaking a cipher with no known key and no certainty about what kind of cipher it even is.
...
![A replica of Harrison's H1 marine chronometer (original 1735), made by Herbert Davies c. 1962. _Source_: Daderot, Clockmakers' Museum, London, via Wikimedia Commons (CC0).](images/Harrison-H1-replica.jpg){#fig-harrison}
:::

The clepsydra is one of humanity's oldest timekeeping instruments, and its flaw was ancient too: water's flow rate shifts with temperature, as the Han-dynasty astronomer Huan Tan noted two thousand years before van Langren. At sea, pitching decks and swinging temperatures would have thrown his copper cones out of sync within days, and the tolerance was brutal: four minutes of drift is one degree of longitude---about 68 km at the latitude of Amsterdam.

Van Langren wrote his cipher in the early 1640s. The pendulum clock was still some fifteen years off; an accurate marine chronometer (@fig-harrison) [@RMG:harrison], more than a century. He was reaching for the best tool of his moment to solve a problem no tool of his era could actually solve.

## Bonus: Reproducing Langren's 1644 Graph

Reproducing a historical graph with modern tools is not just technical imitation.
In my paper "A Brief History of Data Visualization"
...
Those choices, and the difficulty of getting them right, reveal the constraints of the time---and
can expose hidden insights, design flaws such as distorted scales, and the sheer ingenuity of
hand-drawn graphics.
...
almost immediately why a table of twelve longitude values would not have made his point:
a column of numbers, however sorted, tells you nothing at a glance about their spread.
Only on a common scale does it become visible that the estimates are scattered across nearly
the entire plausible range, and *that* is precisely his case to the King.

My first goal was to reproduce something like @fig-1644-overlay: van Langren's estimates overlaid on a modern map. The data are in `HistData::Langren1644`, which also provides the map image; rebuilding it takes three layers: (1) the map itself; (2) city markers for Toledo and Rome; and (3) the dot plot on top---the same approach I've used to re-vision
John Snow's cholera map
and Minard's March on Moscow [@R-HistData].
...
sense means trying to reproduce the *artifact*---the spare white strip across the top of
@fig-1644-overlay, the figure-8
...
labour in 1644. So: how close can we get to the original in @fig-1644-overlay?

The reproduction uses a handful of `ggplot2` techniques worth noting:

- **Period font**: *IM Fell English*, loaded from Google Fonts with `showtext`---digitised
  from type given to Oxford University Press in 1686, forty-two years after *La Verdadera*,
  but as close in spirit as a free font gets.
...
- **Geometry by hand**: the axis, ticks, and fixed labels (*TOLEDO.*, *ROMA*,
  *Grados de la Longitud.*) are drawn one by one with `annotate()` rather than ggplot2's
  axis machinery, giving exact control over each length, position, and angle---much as
  the engraver had.
...
- **Snowman markers**: each of van Langren's figure-8 symbols is two stacked
  `geom_point()` circles, a filled body on the axis and a smaller hollow head above it;
  `coord_fixed()` keeps them round and the strip in the original's roughly 5:1 proportions.
...
#| echo: false
#| fig-cap: "My ggplot2 reconstruction of van Langren's 1644 graph."
#| fig-width: 7
#| fig-asp: !expr 7.5 / 35
#| fig-dpi: 300
library(ggplot2)
...
font_add_google("IM Fell English", "imfell")
showtext_opts(dpi = knitr::opts_current$get("dpi"))   # match the actual device dpi (incl. retina)
showtext_auto()
...
circles. Shown the `ggplot2` code, he might add, "What a lot of trouble to do what I
did lovingly by hand." And he'd be right: the 80/20 rule (or
Pareto Principle) of data graphics says
that 20% of the work gets you 80% of the way; perfecting it takes the other 80%.

The argument the graph was built to make comes through regardless: twelve
authorities, strung across more than twelve degrees, and not one of them close to the
true answer of 16°32'.

Some do go the remaining 80%. For the #30DayChartChallenge in 2021, Cédric Scherer took
up the Dutchman's challenge (@fig-scherer), with period-style typefaces, snowmen assembled
from four stacked symbols, and every marker and label placed in code by hand---down to the
engraver's long s in "Ianfonius." Van Langren might even approve.

![Cédric Scherer's `ggplot2` version of van Langren's graph, #30DayChartChallenge 2021, Day 3: Historical. _Source_: Cédric Scherer [@Scherer:2021].](images/03_historical.png){#fig-scherer}
